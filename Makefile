# Variables
OUTPUT_FILE_GROW_A_GARDEN := ./output/bundle-gag.lua
INPUT_FILE_GROW_A_GARDEN := ./grow-a-garden/main.lua
RELEASE_FILE_GROW_A_GARDEN := ./output/bundle-release.lua
...
.PHONY: run-gag
run-gag: 
	@if [ -f "$(INPUT_FILE_GROW_A_GARDEN)" ]; then \
		lua-bundler -e $(INPUT_FILE_GROW_A_GARDEN) -o $(OUTPUT_FILE_GROW_A_GARDEN) -s; \
	else \
		echo "$(RED)Entry file $(INPUT_FILE_GROW_A_GARDEN) not found!$(NC)"; \
		exit 1; \
	fi

.PHONY: release
release:
	@lua-bundler -e $(INPUT_FILE_GROW_A_GARDEN) -o $(RELEASE_FILE) --release --obfuscate 3
...
run-fish: 
	@if [ -f "$(INPUT_FILE_FISH)" ]; then \
		lua-bundler -e $(INPUT_FILE_FISH) -o $(OUTPUT_FILE_FISH) -s; \
	else \
